...
class Particle(object):
    ...
    def _charge_in_name(self):
        ...
        if self.three_charge == 0 and self.anti_flag == Inv.Same:
            return False  # all quarkonia and the photon
        if self.pdgid in (9000113, 9010113):
            return True  # special particles not yet well-known in the 2018 table
        # Lambda baryons
        if (
            self.pdgid.is_baryon
            and _digit(self.pdgid, Location.Nq2) == 1
            and self.I
            == "0"  # 1st check alone is not sufficient to filter out lowest-ground Sigma's
